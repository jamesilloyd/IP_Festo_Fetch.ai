    ...
    def step(self):
        ...
        if(not self.operations):
            ...
            if(self.lookingForResource):
                ...
                print('OrderAgent {0} - looking for broker'.format(self.unique_id))
                for agent in self.model.schedule.agents:
                    if(agent.agentType == 'broker'):
                        message = {"operation":self.operations[0],'id':self.unique_id}
                        print('Order {0} - message sent to broker {1} - {2}'.format(self.unique_id,agent.unique_id,message))
                ...
                # Order is now looking through the messages it has received
