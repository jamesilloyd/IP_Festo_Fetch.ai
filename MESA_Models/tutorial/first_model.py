...
'''This model works the following way:
    -agents talk to the scheduler when they are "looking for work"
    -schedulers search the agent database for a machine agent that is free and has the right operation
    -if successful search, the scheduler updates the agent and moves it to the machine
    -the machine is then set into inProgress mode
    -when the machine finishes the operation it deletes the operation from the agent's requirements then set's it back to "looking for work"
    -this repeats until all products are complete
'''
